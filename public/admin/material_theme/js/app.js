...
(function ($, window, document, undefined) {
  ...
  Plugin.prototype = {
    ...
    init: function init() {
      ...
      var onStartEvent = function onStartEvent(e) {
        ...
        plugin.dragStart(e.touches ? e.touches[0] : e);
          ...
          onMoveEvent = function onMoveEvent(e) {
        if (plugin.$dragItem) {
          e.preventDefault();
          plugin.dragMove(e.touches ? e.touches[0] : e);
        }
      },
          onEndEvent = function onEndEvent(e) {
        if (plugin.$dragItem) {
          e.preventDefault();
      ...
      if (opt.addBtnSelector) this.addNewListItemListener($(opt.addBtnSelector));else this.addNewListItemListener(this.$instance.find(opt.addBtnClass.dot()));
    },
    /**
     * @dev-since 0.13.29
     ...
     * @param parent
     */
    addNewListItemListener: function addNewListItemListener(addBtn, parent) {
      var _this = this,
          opt = this.options;
     ...
     * @param event
     */
    clickEndEditEventHandler: function clickEndEditEventHandler(item) {
      var _this = this,
          opt = _this.options,
     ...
     */
    clickStartEditEventHandler: function clickStartEditEventHandler(event) {
      ...
      var igniteKeypressEnterEndEditEventHandler = function igniteKeypressEnterEndEditEventHandler(el) {
      ...
      spn.stop().hide(opt.slideAnimationDuration, function () {
        ...
        if (firstInput.get(0).nodeName !== 'SELECT') firstInput.val(spn.text());else firstInput.val(firstInput.find('option:contains(' + spn.text() + ')').val());
     ...
     * @returns {*}
     */
    resolveToken: function resolveToken(token, input) {
     ...
     */
    saveEditBoxInput: function saveEditBoxInput(inputCollection) {
      var _this = this,
          opt = this.options,
          $item;
     ...
     * @param event
     */
    keypressEnterEndEditEventHandler: function keypressEnterEndEditEventHandler(event) {
      var _this = this,
          opt = this.options,
          item = $(event.target).parents(opt.itemClass.dot()).first(),
          edtBox = item.find(opt.editBoxClass.dot()).first(),
    ...
    resolveInputDataEntryByName: function resolveInputDataEntryByName(name) {
      var item = this.$instance;
      opt = this.options, item.find(opt.editBoxClass.dot().join(opt.inputSelector)).each(function (i, input) {
        if (input.getAttribute('name') === name) return $(input).data('name');
      });
    },
    setItemTitle: function setItemTitle(item, title) {
      var _this = this,
          opt = this.options;
      item.find(opt.contentClass.dot().join('span')).first().text(title);
    },
    determineAndSetItemTitle: function determineAndSetItemTitle(item) {
      var _this = this,
          opt = this.options,
          firstInput = item.find(opt.inputSelector).first(),
     ...
     */
    setInputCollectionPlaceholders: function setInputCollectionPlaceholders(item, inputCollection) {
      var _this = this;
      $(inputCollection).each(function (c, input) {
        if (input.nodeName === 'SELECT') {
          ...
          if (selectedOption.length !== 0) selectedOption.attr('selected', 'selected');else if (item.data($(input).attr('name'))) $(input).append('<option value="' + item.data($(input).attr('name')) + '">' + item.data($(input).attr('name')) + '</option>');else return;
        }
     ...
     * @returns {*}
     */
    createNewListItem: function createNewListItem(data) {
      var _this = this,
          el = this.$instance,
          opt = this.options,
     ...
     */
    itemAddChildItem: function itemAddChildItem($parentElement) {
      var _this = this,
          opt = _this.options,
          listElement,
          $newItem = _this.createNewListItem();

      if (!$newItem) return;else if ($parentElement.parents(opt.listClass.dot()).length > opt.maxDepth) {
      ...
      if ($parentElement.children(opt.listClass.dot()).length) $parentElement.children(opt.listClass.dot()).first().append($newItem);else {
    ...
    getHighestId: function getHighestId() {
      var opt = this.options,
          el = this.$instance,
          id = 0;
    ...
    deserialize: function deserialize(data, override) {
      ...
      var processItem = function processItem(i, pref) {
        if (i.children) {
    ...
    expandAll: function expandAll(cb) {
      var list = this,
          recursiveExpand = function recursiveExpand($item) {
     ...
     * @param cb
     */
    collapseAll: function collapseAll(cb) {
      var list = this,
          recursiveCollapse = function recursiveCollapse($item) {
        if (!cb || !cb($item)) return;
        var listBag = $item.children(list.options.listNodeName);
        if (listBag.length) {
     ...
     * @param force
     */
    setParent: function setParent($item, force) {
      var opt = this.options;
     ...
     * @param $item
     */
    unsetParent: function unsetParent($item) {
      var opt = this.options;
    ...
    dragStart: function dragStart(e) {
      ...
      this.$dragItem = $(document.createElement(this.options.listNodeName)).addClass(this.options.listClass + ' ' + this.options.dragClass);
    ...
    dragMove: function dragMove(e) {
      ...
      if (this.options.allowListMerging !== true) {
        if (hasRootChanged && this.options.allowListMerging === false) return;else if (hasRootChanged && $rootListOfPointingElement.data('domenu') && $rootListOfPointingElement.data('domenu').options.allowListMerging.indexOf(this.$instance.attr('id')) === -1) return;
      }
      /**
       * @dev-since 0.48.59
      ...
      if (mouse.isMovingOnXAxis === false && Math.abs(mouse.distYtotal) >= 5) {
        ...
        if (hasRootChanged && opt.allowListMerging === false) {
          return;
        } else if (hasRootChanged && _typeof(opt.allowListMerging) === "object") {
  ...
  function PublicPlugin(plugin, lists) {
    if (!plugin) throw new TypeError('expected object, got ' + (typeof plugin === 'undefined' ? 'undefined' : _typeof(plugin)));
    this._plugin = plugin, this._lists = lists;
  }
  ...
  PublicPlugin.prototype = {
    ...
    parseJson: function parseJson(data, override) {
      var data = data || null,
     ...
     * @returns {PublicPlugin}
     */
    onParseJson: function onParseJson(callback) {
      var _this = this;
     ...
     * @returns {PublicPlugin}
     */
    onItemSetParent: function onItemSetParent(callback) {
      var _this = this;
     ...
     * @returns {PublicPlugin}
     */
    onItemUnsetParent: function onItemUnsetParent(callback) {
      var _this = this;
     ...
     * @returns {PublicPlugin}
     */
    onToJson: function onToJson(callback) {
      var _this = this;
     ...
     * @returns {PublicPlugin}
     */
    onSaveEditBoxInput: function onSaveEditBoxInput(callback) {
      var _this = this;
     ...
     * @returns {PublicPlugin}
     */
    onItemDrag: function onItemDrag(callback) {
      var _this = this;
     ...
     * @returns {PublicPlugin}
     */
    onItemDrop: function onItemDrop(callback) {
      var _this = this;
     ...
     * @returns {PublicPlugin}
     */
    onItemAdded: function onItemAdded(callback) {
      var _this = this;
     ...
     * @returns {PublicPlugin}
     */
    onItemRemoved: function onItemRemoved(callback) {
      var _this = this;
     ...
     * @returns {PublicPlugin}
     */
    onItemStartEdit: function onItemStartEdit(callback) {
      var _this = this;
     ...
     * @returns {PublicPlugin}
     */
    onItemEndEdit: function onItemEndEdit(callback) {
      var _this = this;
     ...
     * @returns {PublicPlugin}
     */
    onItemAddChildItem: function onItemAddChildItem(callback) {
      var _this = this;
     ...
     */
    onCreateItem: function onCreateItem(callback) {
      var _this = this;
     ...
     * @returns {PublicPlugin}
     */
    onItemCollapsed: function onItemCollapsed(callback) {
      var _this = this;
     ...
     * @returns {PublicPlugin}
     */
    onItemExpanded: function onItemExpanded(callback) {
      var _this = this;
     ...
     * @returns {PublicPlugin}
     */
    onItemMaxDepth: function onItemMaxDepth(callback) {
      var _this = this;
     ...
     * @returns {PublicPlugin}
     */
    on: function on(eventBag, callback) {
      var _this = this;
      if ((typeof eventBag === 'undefined' ? 'undefined' : _typeof(eventBag)) === "object") {
    ...
    getListNodes: function getListNodes() {
     ...
     * @returns Object
     */
    getPluginOptions: function getPluginOptions() {
...
(function ($) {
    ...
    $clearFilesBtns.length ? $clearFilesBtns.on('click', function (e) {
        return onClickClearFilesInit(e, $editFilesBtn);
    }) : null;
    ...
    function onChangeUploadFilesInput(e) {
        ...
        !$currentClearFilesBtn.length ? $previewActions.append($clearFilesBtn) : $currentClearFilesBtn.on('click', function (e) {
            return onClickClearFilesBtn(e, $uploadInput, $currentClearFilesBtn, $previewContainer, $noImageInput, $editFilesBtn);
        });

        $clearFilesBtn.on('click', function (e) {
            return onClickClearFilesBtn(e, $uploadInput, $clearFilesBtn, $previewContainer, $noImageInput, $editFilesBtn);
        });

        $currentPreviewImages.length ? $currentPreviewImages.remove() : null;

        $uploadInput[0].files.length ? [].forEach.call($uploadInput[0].files, function (file) {
            var reader = new FileReader();
            reader.onload = function (e) {
                return createPreviewThumbnail(e, $previewRow);
            };
    ...
    function getPreviewImageStructure() {
        var $img = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
        var $cardBody = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';
    ...
    function onClickClearFilesInit(e, $editFilesBtn) {
        ...
        e.stopPropagation();
        var $target = $(e.target);
        var $previewContainer = $target.closest('.pc-cms-image-preview-container');
        var $currentPreviewImages = $previewContainer.find('.pc-cms-single-preview-image');
        var $noImageInput = $previewContainer.find('.pc-cms-no-image');
        $noImageInput.val('yes');
        $currentPreviewImages.remove();
...
(function ($) {
    ...
    var domenu = $menuBuilderTree.domenu(domenuConfig);
    var menuId = parseInt($menuBuilderTree.data('menu-id'));
    ...
    function updateMenuTree($item) {
        $.ajax({
            ...
            error: function error() {
                __WEBPACK_IMPORTED_MODULE_0_toastr___default.a.error('Failed to update menu tree');
    ...
    function onItemRemovedCallback($item) {
        ...
        $.ajax({
            ...
            error: function error() {
                __WEBPACK_IMPORTED_MODULE_0_toastr___default.a.error('Failed to remove menu item');
...
(function ($) {
    ...
    var permissions = $permissionsInput.length ? $permissionsInput.val() !== "" ? JSON.parse($permissionsInput.val()) : {} : null;
    ...
    function setInitialValues() {
        $permissionCheckbox.each(function (index, checkbox) {
            ...
            for (var moduleName in permissions) {
                $checkbox.data('module-name') === moduleName ? permissions[moduleName].permissions && permissions[moduleName].permissions.length ? permissions[moduleName].permissions.forEach(function (per) {
                    return $checkbox.data('route') == per.route && per.allow ? $checkbox.attr('checked', true) : null;
                }) : null : null;
            }
        });
    }
...
(function ($) {
    ...
    var $actionsHeader = $('#header_action_bar');
    var $actionsHeaderBackBtn = $actionsHeader.find('[data-action=close]');
...
(function ($) {
    ...
    var $slugFields = $('.pc-slug-field');

    $slugFields.length && $slugFields.each(function (index, slugField) {
     ...
     * @private
     */
    var _handleConfirm = function _handleConfirm(event, $slugField) {
        event.preventDefault();
        event.stopPropagation();
