...
var codesample = (function () {
  ...
  var getSelectedCodeSample = function (editor) {
    ...
    if ($_akufwm9djcgfo3le.isCodeSample(node)) {
      return node;
    }
    return null;
  };
  ...
  var getLanguages$1 = function (editor) {
    ...
    var customLanguages = $_9v4xdd99jcgfo3kn.getLanguages(editor);
    return customLanguages ? customLanguages : defaultLanguages;
  };
  var getCurrentLanguage = function (editor) {
    var matches;
    var node = $_2qk7b89bjcgfo3kp.getSelectedCodeSample(editor);
    if (node) {
  ...
  var $_f6n25v9ejcgfo3lh = {
    getLanguages: getLanguages$1,
    getCurrentLanguage: getCurrentLanguage
  };

  var $_ezvpgh98jcgfo3kk = {
    open: function (editor) {
      var minWidth = $_9v4xdd99jcgfo3kn.getDialogMinWidth(editor);
      var minHeight = $_9v4xdd99jcgfo3kn.getDialogMinHeight(editor);
      var currentLanguage = $_f6n25v9ejcgfo3lh.getCurrentLanguage(editor);
      var currentLanguages = $_f6n25v9ejcgfo3lh.getLanguages(editor);
      var currentCode = $_2qk7b89bjcgfo3kp.getCurrentCode(editor);
      editor.windowManager.open({
        ...
        onSubmit: function (e) {
          $_2qk7b89bjcgfo3kp.insertCodeSample(editor, e.data.language, e.data.code);
        }
      });
    }
  };
  ...
  var register = function (editor) {
    editor.addCommand('codesample', function () {
      var node = editor.selection.getNode();
      if (editor.selection.isCollapsed() || $_akufwm9djcgfo3le.isCodeSample(node)) {
        $_ezvpgh98jcgfo3kk.open(editor);
      } else {
        editor.formatter.toggle('code');
      }
    });
  };
  var $_2q2dg097jcgfo3ki = { register: register };

  var setup = function (editor) {
    var $ = editor.$;
    editor.on('PreProcess', function (e) {
      $('pre[contenteditable=false]', e.node).filter($_akufwm9djcgfo3le.trimArg($_akufwm9djcgfo3le.isCodeSample)).each(function (idx, elm) {
    ...
    editor.on('SetContent', function () {
      var unprocessedCodeSamples = $('pre').filter($_akufwm9djcgfo3le.trimArg($_akufwm9djcgfo3le.isCodeSample)).filter(function (idx, elm) {
  ...
  var $_f48zww9fjcgfo3lj = { setup: setup };

  var loadCss = function (editor, pluginUrl, addedInlineCss, addedCss) {
    var linkElm;
    var contentCss = $_9v4xdd99jcgfo3kn.getContentCss(editor);
  ...
  var $_8tcs649gjcgfo3lt = { loadCss: loadCss };
  ...
  var $_8dkgbv9hjcgfo3lv = { register: register$1 };

  var addedInlineCss = Cell(false);
  PluginManager.add('codesample', function (editor, pluginUrl) {
    var addedCss = Cell(false);
    $_f48zww9fjcgfo3lj.setup(editor);
    $_8dkgbv9hjcgfo3lv.register(editor);
    $_2q2dg097jcgfo3ki.register(editor);
    editor.on('init', function () {
      $_8tcs649gjcgfo3lt.loadCss(editor, pluginUrl, addedInlineCss, addedCss);
    });
    editor.on('dblclick', function (ev) {
      if ($_akufwm9djcgfo3le.isCodeSample(ev.target)) {
        $_ezvpgh98jcgfo3kk.open(editor);
      }
    });
  });
