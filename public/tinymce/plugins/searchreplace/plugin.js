...
var searchreplace = (function () {
  ...
  var $_b3kqoaihjcgfo4vy = { findAndReplaceDOMText: findAndReplaceDOMText };
  ...
  var markAllMatches = function (editor, currentIndexState, regex) {
    ...
    return $_b3kqoaihjcgfo4vy.findAndReplaceDOMText(regex, node, marker, false, editor.schema);
  };
  ...
  var get = function (editor, currentIndexState) {
    var done = function (keepEditorSelection) {
      return $_1816erifjcgfo4vs.done(editor, currentIndexState, keepEditorSelection);
    };
    var find = function (text, matchCase, wholeWord) {
      return $_1816erifjcgfo4vs.find(editor, currentIndexState, text, matchCase, wholeWord);
    };
    var next = function () {
      return $_1816erifjcgfo4vs.next(editor, currentIndexState);
    };
    var prev = function () {
      return $_1816erifjcgfo4vs.prev(editor, currentIndexState);
    };
    var replace = function (text, forward, all) {
      return $_1816erifjcgfo4vs.replace(editor, currentIndexState, text, forward, all);
    };
    return {
      done: done,
      find: find,
  ...
  var open = function (editor, currentIndexState) {
    ...
    function updateButtonStates() {
      win.statusbar.find('#next').disabled($_1816erifjcgfo4vs.hasNext(editor, currentIndexState) === false);
      win.statusbar.find('#prev').disabled($_1816erifjcgfo4vs.hasPrev(editor, currentIndexState) === false);
    }
    ...
    var win = editor.windowManager.open({
      ...
      onClose: function () {
        editor.focus();
        $_1816erifjcgfo4vs.done(editor, currentIndexState);
        editor.undoManager.add();
      },
      onSubmit: function (e) {
        ...
        if (!text.length) {
          $_1816erifjcgfo4vs.done(editor, currentIndexState, false);
          win.statusbar.items().slice(1).disabled(true);
          return;
        }
        if (last.text === text && last.caseState === caseState && last.wholeWord === wholeWord) {
          if (!$_1816erifjcgfo4vs.hasNext(editor, currentIndexState)) {
            notFoundAlert();
            return;
          }
          $_1816erifjcgfo4vs.next(editor, currentIndexState);
          updateButtonStates();
          return;
        }
        count = $_1816erifjcgfo4vs.find(editor, currentIndexState, text, caseState, wholeWord);
        if (!count) {
          notFoundAlert();
        }
      ...
      buttons: [
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            if (!$_1816erifjcgfo4vs.replace(editor, currentIndexState, win.find('#replace').value())) {
              win.statusbar.items().slice(1).disabled(true);
              currentIndexState.set(-1);
              last = {};
            }
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_1816erifjcgfo4vs.replace(editor, currentIndexState, win.find('#replace').value(), true, true);
            win.statusbar.items().slice(1).disabled(true);
            last = {};
          }
        },
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_1816erifjcgfo4vs.prev(editor, currentIndexState);
            updateButtonStates();
          }
        },
        {
  ...
  var $_432xdiijjcgfo4w9 = { open: open };

  var register = function (editor, currentIndexState) {
    editor.addCommand('SearchReplace', function () {
      $_432xdiijjcgfo4w9.open(editor, currentIndexState);
    });
  };
  var $_dq1iduiijcgfo4w3 = { register: register };
  ...
  var $_1vc6z8ikjcgfo4wd = { register: register$1 };

  PluginManager.add('searchreplace', function (editor) {
    var currentIndexState = Cell(-1);
    $_dq1iduiijcgfo4w3.register(editor, currentIndexState);
    $_1vc6z8ikjcgfo4wd.register(editor, currentIndexState);
    return $_f4ubdriejcgfo4vp.get(editor, currentIndexState);
  });
  var Plugin = function () {
  };
