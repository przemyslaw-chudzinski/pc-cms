...
var textpattern = (function () {
  ...
  var $_edq2tyq3jcgfo7ha = { getPatterns: getPatterns };
  ...
  var patternFromRng = function (patterns, rng, space) {
    ...
    var endPattern = $_b13f23qbjcgfo7hn.findEndPattern(patterns, text, rng.startOffset, delta);
    ...
    var startPattern = $_b13f23qbjcgfo7hn.findPattern(patterns, patternRng.toString());
  ...
  var applyBlockFormat = function (editor, patterns) {
    ...
    if (textBlockElm) {
      ...
      if (firstTextNode) {
        pattern = $_b13f23qbjcgfo7hn.findPattern(patterns, firstTextNode.data);
        if (!pattern) {
          return;
        }
  ...
  function handleEnter(editor, patterns) {
    ...
    wrappedTextNode = $_2yjk51q8jcgfo7hi.applyInlineFormatEnter(editor, patterns);
    if (wrappedTextNode) {
    ...
    $_2yjk51q8jcgfo7hi.applyBlockFormat(editor, patterns);
  }
  function handleInlineKey(editor, patterns) {
    var wrappedTextNode, lastChar, lastCharNode, rng, dom;
    wrappedTextNode = $_2yjk51q8jcgfo7hi.applyInlineFormatSpace(editor, patterns);
    if (wrappedTextNode) {
      dom = editor.dom;
  ...
  var setup = function (editor, patternsState) {
    ...
    editor.on('keydown', function (e) {
      if (e.keyCode === 13 && !VK.modifierPressed(e)) {
        $_frttymq7jcgfo7hf.handleEnter(editor, patternsState.get());
      }
    }, true);
    editor.on('keyup', function (e) {
      if ($_frttymq7jcgfo7hf.checkKeyCode(keyCodes, e)) {
        $_frttymq7jcgfo7hf.handleInlineKey(editor, patternsState.get());
      }
    });
    editor.on('keypress', function (e) {
      if ($_frttymq7jcgfo7hf.checkCharCode(charCodes, e)) {
  ...
  var $_6ex9xq4jcgfo7hc = { setup: setup };

  PluginManager.add('textpattern', function (editor) {
    var patternsState = Cell($_edq2tyq3jcgfo7ha.getPatterns(editor.settings));
    $_6ex9xq4jcgfo7hc.setup(editor, patternsState);
    return $_d3j07tq2jcgfo7h8.get(patternsState);
  });
  var Plugin = function () {
  };
