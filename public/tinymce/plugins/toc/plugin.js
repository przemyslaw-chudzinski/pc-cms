...
var toc = (function () {
  ...
  var $_1gu2cyqkjcgfo7in = { create: create };

  var tocId = $_1gu2cyqkjcgfo7in.create('mcetoc_');
  ...
  var readHeaders = function (editor) {
    var tocClass = $_dcbk4gqjjcgfo7il.getTocClass(editor);
    var headerTag = $_dcbk4gqjjcgfo7il.getTocHeader(editor);
    var selector = generateSelector($_dcbk4gqjjcgfo7il.getTocDepth(editor));
  ...
  var generateTocHtml = function (editor) {
    ...
    return '<div class="' + editor.dom.encode($_dcbk4gqjjcgfo7il.getTocClass(editor)) + '" contenteditable="false">' + html + '</div>';
  };
  var generateTocContentHtml = function (editor) {
    ...
    html += generateTitle($_dcbk4gqjjcgfo7il.getTocHeader(editor), I18n.translate('Table of Contents'));
    for (i = 0; i < headers.length; i++) {
      h = headers[i];
      h.element.id = h.id;
  ...
  var register = function (editor) {
    editor.addCommand('mceInsertToc', function () {
      $_56h40oqfjcgfo7if.insertToc(editor);
    });
    editor.addCommand('mceUpdateToc', function () {
      $_56h40oqfjcgfo7if.updateToc(editor);
    });
  };
  var $_ut9ioqejcgfo7id = { register: register };

  var setup = function (editor) {
    var $ = editor.$, tocClass = $_dcbk4gqjjcgfo7il.getTocClass(editor);
  ...
  var $_dfvrfdqljcgfo7ip = { setup: setup };

  var toggleState = function (editor) {
    return function (e) {
      var ctrl = e.control;
      editor.on('LoadContent SetContent change', function () {
        ctrl.disabled(editor.readonly || !$_56h40oqfjcgfo7if.hasHeaders(editor));
      });
    };
  };
  var isToc = function (editor) {
    return function (elm) {
      return elm && editor.dom.is(elm, '.' + $_dcbk4gqjjcgfo7il.getTocClass(editor)) && editor.getBody().contains(elm);
    };
  };
  ...
  var $_24s72bqmjcgfo7ir = { register: register$1 };

  PluginManager.add('toc', function (editor) {
    $_ut9ioqejcgfo7id.register(editor);
    $_24s72bqmjcgfo7ir.register(editor);
    $_dfvrfdqljcgfo7ip.setup(editor);
  });
  var Plugin = function () {
  };
